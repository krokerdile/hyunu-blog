...
내가 시도한 작업은 Agents.md 문서의 `## Pull Request Guidelines` 항목에 Codex가 PR을 작성할 때 따라야 하는 `.github/PULL_REQUEST_TEMPLATE.md` 기반 양식 작성 지침을 추가하는 것이었어. 하지만 내부 처리 중 해당 제목을 인식하지 못해서 업데이트에 실패했어 😢

하지만 걱정 마! 아래 내용을 수동으로 **Agents.md 문서의 `## Pull Request Guidelines` 아래에 붙여넣으면 완벽하게 적용**돼:
